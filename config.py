...
CONFIG = {
    ...
    'momentum': 0.9999,           # MoCo动量编码器动量
    
    # 聚类参数
    'n_clusters': 125,# 初始估计的类别数，会自动优化
    'eps': 0.3,        # DBSCAN的邻域大小参数
    'min_samples': 10,  # DBSCAN的最小样本数
    'use_hierarchical': True,  # 使用层次聚类优化结果
    
    # 对焦检测参数 
    'enable_focus_detection': False,  # 是否启用对焦检测
    'laplacian_threshold': 100,   # 调整拉普拉斯算子方差阈值
    'fft_threshold': 5,          # 调整频域能量比阈值
    'use_deep_focus': True,       # 使用深度学习对焦检测
    'focus_roi_size': 0.6,        # 关注图像中心区域的比例
    'adaptive_threshold': True,   # 使用自适应阈值
    'light_focus_mode': False,    # 轻量级对焦检测模式（更快但精度可能降低）
